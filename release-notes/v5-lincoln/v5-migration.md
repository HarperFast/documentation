...
First, any `spawn` or `execFile` may only spawn executables or commands that have been registered in the `applications.allowedSpawnCommands` configuration. This provides a much more secure evironment, preventing malicious intrusions. (`exec` is not usable through the substituted module, and `execSync` always throws — see [Child Processes](/reference/v5/components/javascript-environment#child-processes) for the full contract.)

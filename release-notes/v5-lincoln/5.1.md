...
### Custom Content Resources and the `harper+rest://` Scheme (5.1.18)

A component can publish its own content — documentation pages, rendered reports, any text or binary payload — as MCP resources under author-chosen URIs by declaring a static `mcpResources` array on a Resource. Each entry takes a fixed `uri` or an RFC 6570-style `uriTemplate` and names an instance method to serve it ([harper#1613](https://github.com/HarperFast/harper/pull/1613)). Custom resources are served to any session on the profile, including anonymous ones, and access control is the read method's responsibility. See [Custom `mcpResources` opt-in](/reference/v5/mcp/tools-and-resources#custom-mcpresources-opt-in).

`resources/list` now advertises exported application Resources under `harper+rest://<host>:<port>/<path>` instead of `http(s)://`. The MCP specification reserves `https://` for resources a client can fetch directly from the web, and these resolve in-process rather than over the network. Legacy `http(s)://` URIs are still accepted by `resources/read`, `resources/subscribe`, and `resources/unsubscribe`, so a client holding an older listing keeps working.

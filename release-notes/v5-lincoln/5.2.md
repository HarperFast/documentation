...
New `write-transaction-queue-depth` and `read-transaction-queue-depth` metrics report per-thread commit and read-snapshot concurrency against the storage engine, as a throughput/concurrency signal — see [Transaction Queue Depth Metrics](/reference/v5/analytics/overview#transaction-queue-depth-metrics).

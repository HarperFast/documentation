...
### Bounded Startup Install Wait

Startup no longer waits indefinitely for component installs. Listeners open only after startup's component installs finish, so one install that stalled — a git fetch that stops responding, a process stuck in I/O — kept the whole node down with nothing logged after the install began. Startup now waits at most `deployment.startupInstallTimeout` (default 10 minutes; `0` restores the unbounded wait) and logs the components it is still waiting for every 60 seconds ([harper#2072](https://github.com/HarperFast/harper/issues/2072)).

An install still running at the deadline is not cancelled. It finishes in the background while the node serves the version already installed, or without the component if it was never installed — so **during that window the node reports up while that application returns 404 or runs its previous version.** When the install finishes, `get_status` reports `restartRequired: true`; restart the node to load it. A node whose first install of a large component takes longer than 10 minutes now starts without that component after upgrading; raise the timeout or set it to `0` to keep the previous behavior. See [Configuration Options](/reference/v5/configuration/options#deployment).

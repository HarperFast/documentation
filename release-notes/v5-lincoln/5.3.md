...
`harper deploy by_ref=true` builds a package reference from the local git repository, so a deploy records which commit it came from instead of shipping an anonymous snapshot of the working directory. Redeploying the same reference is an exact redeploy, and rolling back is deploying an older one.

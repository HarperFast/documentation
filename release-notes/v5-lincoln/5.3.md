...
MQTT durable subscriptions with QoS 1 or 2 explicitly include superseded versions in live delivery and reconnect catch-up, preserving intermediate retained publications. Non-retained published messages remain independent and are not suppressed by the table version check. See [Subscription options and migration](/reference/v5/resources/resource-api#superseded-record-updates) and [MQTT durable sessions](/reference/v5/mqtt/overview#durable-sessions).

Table subscriptions can also identify the origin node of each event: `subscribe({ includeOrigin: true })` adds `nodeId` and `nodeName` to every event built from an audit record, so a JavaScript consumer can keep one cursor per origin node — the shape replication resumes with — instead of a single cursor over the serving node's merged stream. See [Origin identity](/reference/v5/resources/resource-api#origin-identity).

...
A table can make the decision itself. The `@decide` schema directive calls `models.decide` whenever the source field is written and stores the chosen value on the attribute and, optionally, its probability on a `Float` field, under the same write rules as `@embed`: before the write commits, once per originating write, never on replication or replay. The attribute type selects the closed set — a `String` with `values`, a `Boolean`, or an `Int` with `minimum` and `maximum` — and a review queue becomes an ordinary indexed query on the confidence field ([harper#2839](https://github.com/HarperFast/harper/issues/2839)). See [`@decide`](/reference/v5/database/schema#decide).

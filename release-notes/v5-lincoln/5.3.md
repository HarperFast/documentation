...
Table subscriptions now skip superseded record updates during catch-up with `startTime` or `previousCount`, matching live delivery. A record updated at versions 2, 3, and 4 is delivered only at version 4 if that version is still current. **Consumers that relied on replaying intermediate record versions must set `includeSuperseded: true`.** Raw-event subscriptions retain their existing default; `rawEvents: true` includes superseded events unless `includeSuperseded: false` is explicit.

MQTT durable-session reconnects use the same default: retained publications update records, so catch-up delivers current record state. Non-retained published messages remain independent and are not suppressed by this version check. See [Subscription options and migration](/reference/v5/resources/resource-api#superseded-record-updates) and [MQTT durable sessions](/reference/v5/mqtt/overview#durable-sessions).

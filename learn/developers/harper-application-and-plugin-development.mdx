...
The root path POST requests must be authenticated using the `Authorization` request header. If Harper is configured with the `authentication.authorizeLocal: true` option, then the main admin user (the one created during Harper installation) is automatically added to all requests. Meaning the `Authorization` header can be omitted. Fabric users will always need to provide an `Authorization` header. Harper automatically enables `authentication.authorizeLocal` when installed using the `dev` default config (as was instructed in the previous installation guide).

> Operations API examples throughout this guide will always include an Authorization header value as a reminder for Fabric users as well as for security best practices. The next sections actually demonstrate disabling the `authentication.authorizeLocal` configuration setting for exactly this purpose. Local installation developers can optionally leave this option enabled and omit the Authorization header from their requests for an easier experience.

### Getting Started with Basic Authentication

The simplest authorization scheme is [Basic Authentication](https://developer.mozilla.org/en-US/docs/Web/HTTP/Guides/Authentication#basic_authentication_scheme) which transmits credentials as username/password pairs encoded using base64. Importantly, this scheme does not encrypt credentials. If used over an insecure connection, such as HTTP, they are susceptible to being compromised. Only ever use Basic Authentication over secured connections, such as HTTPS. Even then, its better to upgrade to an encryption based authentication scheme such as JSON Web Tokens (JWT) or certificates; both of which Harper supports and will be covered in later Learn guides.
...
Lets test out the `authorizationValue` by creating a proper Operations API request. First, ensure Harper is running (refer to the previous guide if you need a quick refresher). Using your HTTP client of choice, create a POST request to your running Harper instance with `Authorization` and `Content-Type: application/json` headers, and a JSON body containing `{ "operation": "get_configuration" }`.

> Fabric users, remember to replace `http://localhost` with your Fabric instance URL.

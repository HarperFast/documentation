...
The `GeneratedDescriptionFromProduct` class is the upstream source for the `ProductDescription` cache. Its `get` method fetches the product from the `Product` table (which it extends), builds a prompt, and calls the OpenAI API.
...
class GeneratedDescriptionFromProduct extends tables.Product {
...
tables.ProductDescription.sourcedFrom(GeneratedDescriptionFromProduct);
```
...
## Auto-Invalidating on Product Updates

Rather than requiring a separate manual invalidation request, we can hook into writes to the `Product` table and invalidate the cached description automatically whenever description-relevant fields change.

Export a custom `Product` class that overrides `patch` and `put`. Both methods inspect the incoming data, delegate to `super` to perform the actual write, then call `tables.ProductDescription.invalidate` if any of the fields that feed into the AI prompt have changed:

```javascript
export class Product extends tables.Product {
	static async patch(target, data) {
		const body = await data;
		const result = await super.patch(target, body);
		if (
			...
			tables.ProductDescription.invalidate(target.id ?? target);
		}
		return result;
	}
	...
	static async put(target, data) {
		const body = await data;
		const result = await super.put(target, body);
		tables.ProductDescription.invalidate(target.id ?? target);
		return result;
	}
}
...
`patch` only invalidates when one of the prompt-relevant fields is present in the payload — a `PATCH` that touches only unrelated fields (like an internal `updatedBy` field) won't trigger a regeneration. `put` always invalidates because a full replacement always changes the record.

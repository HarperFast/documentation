...
The `InventoryAPI` class implements `get`, `put`, and `delete`. When write-through is active, Harper calls these methods in the appropriate order.
...
class InventoryAPI extends Resource {
	get headers() {
		return {
...
By default, `put` and `delete` methods do not automatically load the existing cached record. If you need the current record value inside a write method — for example, to validate a field transition or merge partial data — call `ensureLoaded()` first:
...
`ensureLoaded()` checks the local cache first; if the record is absent or stale, it calls `get()` to fetch it from the origin. After `ensureLoaded()`, `this` has the full record properties accessible.

...
function printCoverageThresholds() {
	...
	thresholds.forEach((threshold) => {
		console.log(
			`  ${threshold.percentage}% of views: ${threshold.pathCount} paths (${threshold.pathPercentage}% of all paths)`
		);
	});
	console.log();
}
...
function printViewCountDistribution() {
	...
	console.log(
		`    Total Paths: ${highTrafficPaths.length} (${((highTrafficPaths.length / totalPaths) * 100).toFixed(2)}% of paths)`
	);
	console.log(
		`    Total Views: ${highTrafficViews.toLocaleString()} (${((highTrafficViews / totalViews) * 100).toFixed(2)}% of views)`
	);
	...
	highTrafficRanges.forEach((range) => {
		...
		if (pathsInRange.length > 0) {
			...
			console.log(
				`      ${range.min}-${range.max} views: ${pathsInRange.length} paths (${viewsInRange.toLocaleString()} views)`
			);
		}
	});
	...
	console.log(
		`    Total Paths: ${mediumTrafficPaths.length} (${((mediumTrafficPaths.length / totalPaths) * 100).toFixed(2)}% of paths)`
	);
	console.log(
		`    Total Views: ${mediumTrafficViews.toLocaleString()} (${((mediumTrafficViews / totalViews) * 100).toFixed(2)}% of views)`
	);

	// Low traffic
	console.log('  Low traffic (1-9 views):');
	console.log(
		`    Total Paths: ${lowTrafficPaths.length} (${((lowTrafficPaths.length / totalPaths) * 100).toFixed(2)}% of paths)`
	);
	console.log(
		`    Total Views: ${lowTrafficViews.toLocaleString()} (${((lowTrafficViews / totalViews) * 100).toFixed(2)}% of views)`
	);

	// Zero views
	console.log('  Zero views:');
	console.log(
		`    Total Paths: ${zeroTrafficPaths.length} (${((zeroTrafficPaths.length / totalPaths) * 100).toFixed(2)}% of paths)`
	);
	console.log();
}
...
function printLongTailAnalysis() {
	...
	console.log(
		`    Count: ${singleDigitViews.length} (${((singleDigitViews.length / totalPaths) * 100).toFixed(2)}% of paths)`
	);

...
The probability is whatever the configured backend reports. With the built-in [generative adapter](../models/backends#generative-decision-adapter) under its default `scoring: auto`, a generative model that exposes token log-probabilities ([OpenAI](../models/backends#openai)) gives a normalized log-probability score from one scoring call per `@decide` attribute, and any other model gives the vote fraction over `samples` completions. Neither is a calibrated probability, so a threshold such as `routeConfidence < 0.7` is a review-queue rule rather than a guarantee. Cost follows the same split: the `Ticket` type above makes three decisions on every write that carries `body`, three scoring calls when scoring or fifteen completions at the default `samples` when voting, including a write that table validation later rejects. For a high write rate, route the directive's model name to a scoring-capable generative model or a single-call decision backend, or lower `samples`.

...
- `containment` — The method used to load modules (and isolate the application). The default is 'vm', which uses Node's VM to load modules. This can also be set to 'none' (use standard Node module loader), or `compartment`, which uses the `ses` implementation of the proposed `Compartment` functionality.
- `dependencyContainment` — The VM module loader can be used to load packages/dependencies (installed as `dependencies` from the package.json). The default is 'auto', which only use the VM module loader if the package specifies `harper` as a dependency. This can also be set to `true` to always use the VM loader or `false` to always native module loader for packages.

...
| `backend`, `model`, `signature`, `configHash`, `instructionsHash` | Who answered and under what: the backend, the logical model name, the backend's scoring configuration when it reports one, the identity of the `models` configuration actually installed, and the hash of the per-call `instructions` when any were given |
